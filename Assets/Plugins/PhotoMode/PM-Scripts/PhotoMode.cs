...
namespace PhotoMode
{
    ...
    public class PhotoMode : MonoBehaviour
    {
        ...
        [SerializeField] private EventSystem photoModeEventSystem;
        private PhotoModeInputs photoModeInputs;
        private CinemachineVirtualCameraBase photoModeCamera;
        ...
        [Header("VCamera")] 
        [SerializeField] private GameObject m_vCam;
        ...
        private void Awake()
        {
            ...
            _VcamType = m_vCam == null ? typeof(CinemachineFreeLook) : typeof(CinemachineVirtualCamera);
            photoModeCamera = m_vCam == null ? GetComponentInChildren<CinemachineFreeLook>() : m_vCam.GetComponent<CinemachineVirtualCamera>();
            photoModeCameraOffset = m_vCam == null ? photoModeCamera.GetComponent<CinemachineCameraOffset>() : m_vCam.GetComponent<CinemachineCameraOffset>();
            ...
            /*print($"Lens D : {photoModeCamera.m_Lens.Dutch}");
            print($"camDist : {photoModeCameraOffset.m_Offset.z}");
            print($"Focus : {dof.focusDistance.value}");
            print($"Aperture : {dof.aperture.value}");
            print($"PostExposure : {colorAdj.postExposure.value}");
            print($"Contrast : {colorAdj.contrast.value}");
            print($"Saturation : {colorAdj.saturation.value}");
            print($"VignetteColor : {vignetteColor.a}");*/
        }
        ...
        {
            if(_VcamType == typeof(CinemachineFreeLook))
                ((CinemachineFreeLook)photoModeCamera).m_Lens.Dutch = (slider.value - slider.minValue) / (slider.maxValue - slider.minValue) * (viewRoll.max - viewRoll.min) + viewRoll.min;
            else 
                ((CinemachineVirtualCamera)photoModeCamera).m_Lens.Dutch = (slider.value - slider.minValue) / (slider.maxValue - slider.minValue) * (viewRoll.max - viewRoll.min) + viewRoll.min;
        }
        ...
        public void ResetValues()
        {
            ...
            if (_VcamType == typeof(CinemachineFreeLook))
            {
                ((CinemachineFreeLook)photoModeCamera).m_XAxis.Value = photoModeCameraXAxis;
                ((CinemachineFreeLook)photoModeCamera).m_XAxis.Value = photoModeCameraXAxis;
            }
